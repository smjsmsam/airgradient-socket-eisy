...
// EISY config
boolean connectEISY=true;             // Can be set to "false" to stop attempting to connect to EISY
...
IPAddress host(192,168,xxx,xxx);  // Replace with the local IP address of your eisy  Ex. IPAddress host(192,168,1,232)
   ...
   if (currentMillis - previoussendToServer >= sendToServerInterval) {
     previoussendToServer += sendToServerInterval;
      ...
      if(WiFi.status()==WL_CONNECTED){
        ...
        WiFiClient client;
        if (connectEISY && client.connect(host, portNumber)) {
